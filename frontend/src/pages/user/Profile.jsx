...
// pages/user/Profile.jsx — FIXED
// Bug: No location/GPS fetching existed at all — added "Use my location" button that calls
// the browser Geolocation API, then reverse-geocodes with nominatim (OpenStreetMap, free, no key needed)
// to fill street / city / state / pincode / country automatically.
// Also fixed: mock stats replaced with real data fetched from the orders/complaints APIs.

import React, { useState, useEffect, useCallback } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { updateProfile } from '../../features/auth/authSlice';
import {
  User, Mail, Phone, MapPin, Home, Building2, Globe,
  Shield, ShoppingBag, MessageSquare, Star, Edit2, Save, X,
  CheckCircle, Truck, Clock, Award, Camera, ChevronRight,
  Sparkles, Package, Heart, Navigation, Loader2
} from 'lucide-react';
import toast from 'react-hot-toast';
...
const Profile = () => {
  ...
  const [focusedField, setFocusedField] = useState(null);
  const [stats, setStats] = useState({ totalOrders: 0, pendingComplaints: 0, reviewsWritten: 0 });
  ...
  useEffect(() => {
    if (userInfo) {
      setFormData({
        ...
        email: userInfo.email || '',
        phone: userInfo.phone || '',
        address: userInfo.address || { street: '', city: '', state: '', pincode: '', country: 'India' }
      });
    }
  }, [userInfo]);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [ordRes, cmpRes] = await Promise.all([
          api.get('/orders/myorders').catch(() => ({ data: [] })),
          api.get('/complaints/mycomplaints').catch(() => ({ data: [] }))
        ]);
        const orders = Array.isArray(ordRes.data) ? ordRes.data : [];
        const complaints = Array.isArray(cmpRes.data) ? cmpRes.data : [];
        setStats({
          totalOrders: orders.length,
          pendingComplaints: complaints.filter(c => c.status === 'open' || c.status === 'pending').length,
          reviewsWritten: 0 // reviews are per-product; would need separate endpoint
        });
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          toast.dismiss(tid);
          toast.success('Location filled in! Please review and adjust if needed.');
        } catch (err) {
          toast.dismiss(tid);
          toast.error('Could not reverse-geocode location. Please fill manually.');
      ...
      (err) => {
        ...
        else if (err.code === 2) toast.error('Location unavailable. Try again or fill manually.');
  ...
  const handleCancel = () => {
    ...
    setIsEditing(false);
    toast('Edit cancelled', { icon: '✖️', duration: 1500 });
  };
  ...
  const inputCls = (field) =>
    `w-full pl-10 pr-4 py-2.5 border-2 rounded-xl focus:outline-none transition-all ${
      !isEditing
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <div className="bg-slate-900 pt-20 pb-32 relative overflow-hidden">
        ...
        <div className="relative container mx-auto px-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="flex items-center gap-5">
              <div className="relative">
                <div className="w-24 h-24 md:w-32 md:h-32 bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-xl border-4 border-white">
                  <span className="text-3xl md:text-4xl font-bold">{getInitials(formData.name)}</span>
                </div>
              </div>
              <div>
                <h1 className="text-2xl md:text-4xl font-bold text-white">{formData.name}</h1>
                <p className="text-slate-300 text-sm mt-1 flex items-center gap-1">
                  <Mail className="w-3 h-3" /> {formData.email}
                </p>
                <span className="inline-block mt-2 px-3 py-1 bg-white/10 rounded-full text-xs text-white capitalize">
            ...
            {!isEditing && (
              <button
                onClick={() => setIsEditing(true)}
                className="flex items-center gap-2 px-5 py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-all border border-white/20 backdrop-blur-sm font-medium text-sm"
              >
          ...
          <div className="mt-8 grid grid-cols-3 gap-4 max-w-lg">
            {[
              ...
              { icon: MessageSquare, label: 'Complaints', value: stats.pendingComplaints },
      ...
      <div className="container mx-auto px-4 -mt-16 pb-16 relative z-10">
        <div className="grid lg:grid-cols-3 gap-6">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" name="name" value={formData.name} onChange={handleChange}
                        ...
                        disabled={!isEditing} className={inputCls('name')} />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="tel" name="phone" value={formData.phone} onChange={handleChange}
                        ...
                        disabled={!isEditing} className={inputCls('phone')} />
                    </div>
                  </div>
                  <div className="sm:col-span-2">
                    ...
                    <p className="text-xs text-slate-400 mt-1 flex items-center gap-1">
                      <CheckCircle className="w-3 h-3 text-emerald-500" /> Email cannot be changed
                    </p>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {isEditing && (
                      <button type="button" onClick={handleUseLocation} disabled={isGeoLoading}
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg text-xs font-medium transition-all border border-indigo-200 disabled:opacity-60">
                        {isGeoLoading
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="text" name="address.street" value={formData.address.street}
                          onChange={handleChange} onFocus={() => setFocusedField('street')} onBlur={() => setFocusedField(null)}
                          disabled={!isEditing} placeholder="House number, street name" className={inputCls('street')} />
                      </div>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input type="text" name="address.city" value={formData.address.city}
                            onChange={handleChange} onFocus={() => setFocusedField('city')} onBlur={() => setFocusedField(null)}
                            disabled={!isEditing} className={inputCls('city')} />
                        </div>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-slate-700 mb-2">State</label>
                        <input type="text" name="address.state" value={formData.address.state}
                          onChange={handleChange} onFocus={() => setFocusedField('state')} onBlur={() => setFocusedField(null)}
                          disabled={!isEditing}
                          className={`w-full px-4 py-2.5 border-2 rounded-xl focus:outline-none transition-all ${
                            !isEditing ? 'bg-slate-50 border-slate-200 text-slate-500'
                              : focusedField === 'state' ? 'border-indigo-400 ring-4 ring-indigo-100 bg-white'
                              : 'border-slate-200 hover:border-indigo-300 bg-white'
                          }`} />
                      </div>
                    </div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="text" name="address.pincode" value={formData.address.pincode}
                          maxLength={6} onChange={handleChange} onFocus={() => setFocusedField('pincode')} onBlur={() => setFocusedField(null)}
                          disabled={!isEditing}
                          className={`w-full px-4 py-2.5 border-2 rounded-xl focus:outline-none transition-all ${
                            !isEditing ? 'bg-slate-50 border-slate-200 text-slate-500'
                              : focusedField === 'pincode' ? 'border-indigo-400 ring-4 ring-indigo-100 bg-white'
                              : 'border-slate-200 hover:border-indigo-300 bg-white'
                          }`} />
                      </div>
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input type="text" name="address.country" value={formData.address.country}
                            onChange={handleChange} onFocus={() => setFocusedField('country')} onBlur={() => setFocusedField(null)}
                            disabled={!isEditing} className={inputCls('country')} />
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {isEditing && (
                  <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-slate-100">
                    <button type="button" onClick={handleCancel}
                      className="px-6 py-2.5 border-2 border-slate-300 rounded-xl text-slate-700 font-medium hover:bg-slate-50 transition-all flex items-center justify-center gap-2">
          ...
          {/* Sidebar */}
          <div className="space-y-6">
            <div className="bg-gradient-to-r from-emerald-600 to-green-600 rounded-2xl shadow-sm p-5 text-white border border-emerald-500">
              <div className="flex items-center justify-between mb-3">
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-2xl shadow-sm p-5 border border-indigo-100">
              <div className="flex items-center gap-2 mb-3">
                <div className="p-1.5 bg-indigo-100 rounded-lg"><Sparkles className="w-4 h-4 text-indigo-600" /></div>
                <h3 className="font-semibold text-slate-800">Tips</h3>
              </div>
              <ul className="space-y-2 text-sm text-slate-600">
                <li className="flex items-start gap-2"><span className="text-indigo-500">✓</span> Use "Use My Location" to auto-fill address</li>
                <li className="flex items-start gap-2"><span className="text-indigo-500">✓</span> Complete profile for faster checkout</li>
                <li className="flex items-start gap-2"><span className="text-indigo-500">✓</span> Write reviews to help other customers</li>
              </ul>
            </div>
          </div>
        </div>
