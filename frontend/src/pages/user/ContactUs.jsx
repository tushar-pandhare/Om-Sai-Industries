...
import {
  ...
  Minimize2, Maximize2, Users, Wifi, WifiOff, CornerDownLeft
} from 'lucide-react';
...
const ContactUs = () => {
  ...
  // FIX: track pending temp IDs so message_sent can replace them
  const pendingMap = useRef({}); // tempId → true
  ...
  useEffect(() => {
    ...
    // FIX: message_sent reconciles the optimistic (tempId) message with the real saved one.
    // Without this, the customer saw their own message twice:
    //   1. The optimistic copy (tempId)
    //   2. The real copy echoed back via receive_message (which was checking .from._id only)
    const handleSent = (savedMsg) => {
      setMessages(prev =>
        prev.map(m => pendingMap.current[m._id] ? { ...savedMsg, _id: savedMsg._id } : m)
      );
      // Clear all pending entries (only one message sent at a time)
      pendingMap.current = {};
    };

    // FIX: receive_message now only adds messages FROM someone else.
    // Previously it added every receive_message including echoes of your own sends,
    // causing the message to appear twice (once optimistic, once from socket echo).
    const handleReceive = (msg) => {
      // Ignore messages sent by the current user — those are handled by message_sent
      if (msg.from?._id === userInfo?._id || msg.from?._id?.toString() === userInfo?._id) return;

      setMessages(prev => {
        if (prev.some(m => m._id === msg._id)) return prev; // dedup by real _id
        return [...prev, msg];
      });
  ...
  const handleSendMessage = (e) => {
    ...
    socket.emit('send_message', { message: optimistic.message, type: 'text' });
  };

  const handleTypingInput = (e) => {
    setMessage(e.target.value);
    if (e.target.value.length > 0 && socket && conversation?.user) {
  ...
  const toggleFullscreen = () => {
    if (!isFullscreen) chatRef.current?.requestFullscreen?.().catch(() => {});
  ...
  const handleContactSubmit = async (e) => {
    ...
    try {
      await messageAPI.sendContact(formData);
  ...
  if (userInfo) {
    ...
    return (
      <div className="min-h-screen bg-[#0f1117] text-white" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        {!isFullscreen && (
          <div className="relative bg-[#13151f] border-b border-white/5 pt-16 pb-24 overflow-hidden">
            ...
            <div className="relative container mx-auto px-4 text-center">
              ...
              <p className="text-zinc-500 max-w-lg mx-auto">Real-time help from our team — usually replies within minutes.</p>
            </div>
          </div>
        )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {isConnected
                        ? <><Wifi className="h-3 w-3 text-emerald-400" /><span className="text-xs text-zinc-500">Online · Usually replies in minutes</span></>
                        : <><WifiOff className="h-3 w-3 text-zinc-600" /><span className="text-xs text-zinc-600">Connecting…</span></>
                      }
                    </div>
                  </div>
                </div>
                ...
                {loading ? (
                  ...
                  <div className="max-w-2xl mx-auto space-y-1">
                    {messages.map((msg, idx) => {
                      ...
                      const isMine = msg.from?._id === userInfo._id || msg.from?._id?.toString() === userInfo._id;
                      return (
                        <React.Fragment key={msg._id || idx}>
                          ...
                          <div className={`flex items-end gap-2 ${isMine ? 'justify-end' : 'justify-start'}`}>
                            {!isMine && (
                              <div className="w-7 h-7 flex-shrink-0 rounded-full bg-gradient-to-br from-violet-600 to-fuchsia-600 flex items-center justify-center text-xs font-bold mb-1">S</div>
                            )}
                            <div className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed
                              ${isMine ? 'bg-violet-600 text-white rounded-br-sm' : 'bg-[#1e2130] text-zinc-200 rounded-bl-sm'}
                              ${msg._isPending ? 'opacity-70' : 'opacity-100'}
                            `}>
                    ...
                    {adminTyping && (
                      <div className="flex items-end gap-2 justify-start">
                        <div className="w-7 h-7 flex-shrink-0 rounded-full bg-gradient-to-br from-violet-600 to-fuchsia-600 flex items-center justify-center text-xs font-bold">S</div>
              ...
              <div className="flex-shrink-0 px-5 py-4 border-t border-white/5 bg-[#13151f]">
                <form onSubmit={handleSendMessage}>
                  <div className="flex items-end gap-3 max-w-2xl mx-auto">
                    ...
                    />
                    <button type="submit" disabled={!message.trim() || !conversation?.user}
  ...
  return (
    <div className="min-h-screen bg-[#0f1117] text-white flex items-center justify-center py-20 px-4" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" required value={formData.name} onChange={e => setFormData(p => ({ ...p, name: e.target.value }))}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-zinc-200 placeholder-zinc-700 focus:outline-none focus:ring-1 focus:ring-violet-500/50" placeholder="John Doe" />
              </div>
              <div>
                ...
                <input type="email" required value={formData.email} onChange={e => setFormData(p => ({ ...p, email: e.target.value }))}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-zinc-200 placeholder-zinc-700 focus:outline-none focus:ring-1 focus:ring-violet-500/50" placeholder="john@example.com" />
              </div>
            </div>
            <div>
              <label className="text-xs text-zinc-500 mb-1.5 block">Phone (optional)</label>
              <input type="tel" value={formData.phone} onChange={e => setFormData(p => ({ ...p, phone: e.target.value }))}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-zinc-200 placeholder-zinc-700 focus:outline-none focus:ring-1 focus:ring-violet-500/50" placeholder="+91 1234567890" />
            </div>
            <div>
              ...
              <textarea required rows={4} value={formData.message} onChange={e => setFormData(p => ({ ...p, message: e.target.value }))}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-zinc-200 placeholder-zinc-700 focus:outline-none focus:ring-1 focus:ring-violet-500/50 resize-none" placeholder="Describe your inquiry…" />
            </div>
