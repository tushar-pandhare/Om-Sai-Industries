...
import { 
  Star, Search, Filter, Trash2, User, Package, Calendar, 
  TrendingUp, MessageSquare, Tag, Info, Loader2, 
...
const ReviewCard = ({ review, onDelete }) => {
  const getTypeBadge = () => {
    if (!review.isProductReview) {
      return { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200', icon: MessageSquare, label: 'Feedback' };
    }
    if (review.source === 'embedded') {
      return { bg: 'bg-amber-50', text: 'text-amber-700', border: 'border-amber-200', icon: Info, label: 'Legacy' };
    }
    return { bg: 'bg-blue-50', text: 'text-blue-700', border: 'border-blue-200', icon: Package, label: 'Review' };
  };
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-5 hover:shadow-lg transition-all duration-300">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          {/* Badges */}
          <div className="flex flex-wrap items-center gap-2 mb-3">
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium border ${badge.bg} ${badge.text} ${badge.border}`}>
              <BadgeIcon className="h-3 w-3" />
              {badge.label}
            </span>
            {review.category && (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-purple-50 text-purple-700 border border-purple-200 rounded-lg text-xs">
                <Tag className="h-3 w-3" />
                {review.category}
              </span>
            )}
          ...
          {/* User Info */}
          <div className="flex gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-gray-600 to-gray-700 rounded-xl flex items-center justify-center text-white font-semibold flex-shrink-0 shadow-md">
              {review.userName?.charAt(0).toUpperCase() || 'U'}
            </div>
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mb-1">
        ...
        <button
          onClick={() => onDelete(review._id, review.userName, review.isProductReview)}
          className="p-2 text-gray-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all duration-200 self-start"
          title="Delete"
        >
          <Trash2 className="h-4 w-4" />
        </button>
...
const Reviews = () => {
  ...
  const handleDelete = async (id, userName, isProductReview) => {
    if (window.confirm(`Delete ${!isProductReview ? 'feedback' : 'review'} from "${userName}"? This action cannot be undone.`)) {
      const result = await dispatch(deleteReview(id));
      if (deleteReview.fulfilled.match(result)) {
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Customer Feedback & Reviews</h1>
                <p className="text-sm text-gray-500 mt-1">Monitor and manage customer feedback and product reviews</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 mt-4 ml-12">
            ...
            <span className="text-gray-900 font-medium">Reviews & Feedback</span>
          </div>
        </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none bg-white"
                    >
                      <option value="all">All Types</option>
                      <option value="product-review">Product Reviews</option>
                      <option value="general-feedback">General Feedback</option>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-500 text-sm">
              {searchTerm || filter !== 'all' || typeFilter !== 'all' 
                ...
                : "No customer feedback or reviews available"}
            </p>
            {(searchTerm || filter !== 'all' || typeFilter !== 'all') && (
              <button
