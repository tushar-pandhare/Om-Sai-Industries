...
import { 
  Edit, Trash2, Eye, Search, Filter, ChevronLeft, ChevronRight, 
  Package, Plus, Grid3x3, List, TrendingUp, AlertCircle, 
  CheckCircle, XCircle, ImageOff, DollarSign, Tag, ShoppingBag,
  AlertTriangle, Box
} from 'lucide-react';
import toast from 'react-hot-toast';
...
const ProductImage = ({ src, alt, size = 'w-10 h-10' }) => {
  ...
  if (!src || imgError) {
    return (
      <div className={`${size} bg-gradient-to-br from-gray-100 to-gray-200 rounded-xl flex items-center justify-center`}>
  ...
  return (
    <img
      src={src}
      alt={alt}
      className={`${size} object-cover rounded-xl border border-gray-200 shadow-sm`}
      onError={() => setImgError(true)}
    />
  );
};
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          {/* Breadcrumb - Consistent with AddProduct */}
        ...
        {/* Stats Grid - Clean cards matching AddProduct style */}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-all duration-300">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-2">₹{(stats.totalValue / 100000).toFixed(1)}L</p>
              </div>
        ...
        {/* Search and Filters Bar - Consistent styling */}
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="flex gap-2">
                ...
                <div className="flex rounded-xl border border-gray-200 overflow-hidden">
                  <button
                    onClick={() => setViewMode('grid')}
                    className={`p-2.5 transition-all duration-200 ${viewMode === 'grid' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
                  >
                    <Grid3x3 className="h-4 w-4" />
                  </button>
                  <button
                    onClick={() => setViewMode('list')}
                    className={`p-2.5 transition-all duration-200 ${viewMode === 'list' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
                  >
                    <List className="h-4 w-4" />
                  </button>
                </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {currentProducts?.map((product) => (
              <div key={product._id} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group">
                <div className="relative">
                  <div className="aspect-square bg-gradient-to-br from-gray-50 to-gray-100 relative">
                    ...
                    {product.discountPercent > 0 && (
                      <div className="absolute top-3 left-3">
                        <span className="bg-gradient-to-r from-blue-600 to-blue-700 text-white text-xs font-bold px-2 py-1 rounded-lg shadow-md">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                    ...
                    <div className="flex gap-1">
                      ...
                      <button onClick={() => handleDelete(product._id, product.name)} className="p-1.5 text-gray-400 hover:text-red-600 transition-colors">
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </div>
                  </div>
