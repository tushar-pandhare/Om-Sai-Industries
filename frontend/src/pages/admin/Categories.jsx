...
import React, { useState, useEffect } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { X, Edit, Trash2, Plus, Image as ImageIcon, Package, ArrowLeft, Upload, Tag } from 'lucide-react';
import { Link } from 'react-router-dom';
import { fetchCategories, createCategory, updateCategory, deleteCategory } from '../../features/categories/categorySlice';
import toast from 'react-hot-toast';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header Section - Consistent with AddProduct */}
        <div className="mb-8">
        ...
        {/* Stats Summary - Consistent card style */}
        {categories?.length > 0 && (
          <div className="mb-6">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-500 text-sm mb-4">Create your first category to start organizing products</p>
            <button
              onClick={() => handleOpenModal()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories?.map((category) => (
              <div key={category._id} className="group bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300">
                <div className="relative h-48 bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
                  ...
                  <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-200">
                    <button
                      onClick={() => handleOpenModal(category)}
                      className="p-2 bg-white rounded-lg shadow-md hover:bg-blue-50 transition-colors text-blue-600 transform hover:scale-105"
                      title="Edit Category"
                    >
                      <Edit className="h-4 w-4" />
                    </button>
                    <button
                      onClick={() => handleDelete(category._id, category.name)}
                      className="p-2 bg-white rounded-lg shadow-md hover:bg-red-50 transition-colors text-red-600 transform hover:scale-105"
                      title="Delete Category"
                    >
                      <Trash2 className="h-4 w-4" />
                    </button>
        ...
        {/* Modal - Consistent with AddProduct modal style */}
        {isModalOpen && (
              ...
              <form onSubmit={handleSubmit}>
                <div className="p-6 space-y-5">
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-200 border-dashed rounded-xl hover:border-blue-400 transition-all duration-200">
                      <div className="space-y-2 text-center">
                        <Upload className="mx-auto h-12 w-12 text-gray-400" />
                        <div className="flex text-sm text-gray-600 justify-center">
                          <label htmlFor="category-image" className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 transition-colors">
                            <span>Upload an image</span>
                            <input
                              id="category-image"
                              name="category-image"
                    ...
                    <div>
                      <input
                        ...
                        onChange={handleChange}
                        disabled={!!imageFile}
                        className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none disabled:bg-gray-50 disabled:cursor-not-allowed"
                        placeholder="https://example.com/category-image.jpg"
                      />
                      {imageFile && (
                ...
                <div className="flex justify-end gap-3 p-6 border-t border-gray-100 bg-gray-50 rounded-b-2xl">
                  <button
                    type="button"
                    onClick={handleCloseModal}
                    className="px-4 py-2 border border-gray-200 rounded-xl font-medium text-gray-700 hover:bg-white transition-all duration-200"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={uploading}
                    className="px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white rounded-xl font-medium transition-all duration-200 shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                  >
                    {uploading ? (
                      <>
                        <div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent"></div>
