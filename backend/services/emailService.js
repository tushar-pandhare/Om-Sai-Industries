...
class EmailService {
  ...
  static async sendWelcomeEmail(user) {
    try {
      ...
      // Also notify super admin about new user registration (wrap in try-catch to avoid breaking main flow)
      try {
        await this.sendSuperAdminNotification('new_user', { user });
      } catch (adminError) {
        console.error('⚠️ Failed to send super admin notification:', adminError.message);
        // Don't throw error - welcome email was already sent successfully
      }
  ...
  static async sendOrderConfirmation(order, user) {
    try {
      ...
      try {
        ...
        console.error('⚠️ Failed to send super admin notification for order:', adminError.message);
      }
  ...
  static async sendOrderStatusUpdate(order, user, oldStatus, updatedBy = null) {
    try {
      ...
      try {
        ...
        console.error('⚠️ Failed to send super admin notification for status change:', adminError.message);
      }
  ...
  static async sendOrderCancellation(order, user, cancelledBy = null) {
    try {
      ...
      try {
        ...
        console.error('⚠️ Failed to send super admin notification for cancellation:', adminError.message);
      }
  ...
  // Super Admin Notification Handler
  static async sendSuperAdminNotification(type, data) {
    try {
      // Ensure SUPER_ADMIN_EMAIL is set, fallback to ADMIN_EMAIL
      const superAdminEmail = process.env.SUPER_ADMIN_EMAIL || process.env.ADMIN_EMAIL;
      
      // Don't send if no admin email is configured
      if (!superAdminEmail) {
        console.log('⚠️ No admin email configured, skipping super admin notification');
        return false;
      }
      ...
      await transporter.sendMail(mailOptions);
      console.log(`✅ Super admin notification sent (${type}) to ${superAdminEmail}`);
      return true;
    } catch (error) {
      console.error(`❌ Error sending super admin notification (${type}):`, error.message);
      return false;
  ...
  static async sendLowStockAlert(product) {
    try {
      ...
      if (!superAdminEmail) {
        console.log('⚠️ No admin email configured, skipping low stock alert');
        return false;
      }
